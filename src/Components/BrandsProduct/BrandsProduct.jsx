...
import toast from 'react-hot-toast';
  ...
  const toggleLike = (productId) => {
    ...
    const res  =  axios.get(`https://ecommerce.routemisr.com/api/v1/products?brand=${brandId}`)
    .then((res)=>{
        setProducts(res.data.data);
        setLoading(false);
        

        // console.log('getBrandProducts',res);
    })
    .catch((err)=>{

         console.error('Error fetching products:', err);
    })  }
    // const {isLoading , isError , data} = useQuery(`CategoriesProducts-${brandId}` , getBrandProducts)
  ...
  async function removeProductFromWish(id) {
    ...
    if(products.length === 0) {
     ...
     return <>
      ...
      {products.length > 0 && <h2 className='text-center'>{products[0].brand.name}</h2>}
